...
class ChannelConfig {
    ...
    if (!this.setupEvents) {
      ...
      bot.on('channelCreate', channel => {
        ...
        if (!channel.name || !channel.guild.name || !channel.id) {
          ...
          logger.log(
            'Error when adding channel config to database',
            util.inspect(channel, { depth: 2 }),
            'YELLOW'
          );
        } else {
          this.add(channel).catch(e => bot.emit('error', e));
  ...
  deleteFromGuild(guildID) {
    return Promise.all(
      this._data
        ...
        .map(c => {
          Log.info(
            `ChannelConf | Deleting "${c.guildName}"'s #${c.channelName} (${c.channelID})`
          );
  ...
  add(channel) {
    ...
    if (channel && channel.id && this.get(channel.id)) {
    ...
    const conf = {
      ...
      repos: [],
      prefix: `GL! `,
      disabledEvents: ['merge_request/update', 'pipeline'],
    };
  ...
  addFromGuild(guild) {
    return Promise.all(
      guild.channels
        ...
        .map(c => {
          Log.info(
            `ChannelConf | Adding "${c.guildName}"'s #${c.channelName} (${c.channelID})`
          );
          return this.add(c);
        })
    ).then(() => this);
