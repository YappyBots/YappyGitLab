...
const addGuilds = async (bot) => {
    ...
    queue.addAll(bot.guilds.cache.filter((g) => g && g.id && !guilds.get(g.id)).map((g) => () => Guild.create(g)));
...
const addChannels = async (bot) => {
    ...
    queue.addAll(bot.channels.cache.filter((ch) => ch && ch.id && !channels.get(ch.id)).map((ch) => () => Channel.create(ch)));
