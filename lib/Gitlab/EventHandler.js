...
class Events {
  ...
  use(data, eventName) {
    ...
    const noteableType =
      data.object_attributes && data.object_attributes.noteable_type
        ? data.object_attributes.noteable_type.toLowerCase()
        : '';
    eventName = eventName
      .replace(` Hook`, '')
      .replace(/ /g, '_')
  ...
  parseEmbed(embed, data) {
        ...
        if (embed.color)
          embed.color =
            typeof embed.color === 'string'
              ? parseInt(`0x${embed.color.replace(`0x`, ``)}`, 16)
              : embed.color;
        break;
    }
    ...
    embed.author = {
      name: data.user
        ? data.user.username
        : data.user_username || data.user_name,
      icon_url:
        avatar && avatar.startsWith('/')
          ? `https://gitlab.com${avatar}`
          : avatar,
    };
    embed.footer = {
    ...
    embed.url =
      embed.url ||
      (data.object_attributes && data.object_attributes.url) ||
      (data.project && data.project.web_url);
    embed.timestamp = new Date();

    if (embed.description)
      embed.description =
        embed.description.slice(0, 1000) +
        (embed.description.length > 1000 ? '...' : '');
