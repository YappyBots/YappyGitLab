...
class MergeRequestUpdate extends EventResponse {
  constructor(...args) {
    super(...args, {
      description: 'This event gets fired when a merge request is updated',
    });
  }
  ...
  embed(data) {
    ...
    const lastCommitAuthor =
      lastCommit.author.name || data.user_username || data.user_name;
    return {
      color: 0x149617,
      title: `updated merge request #${mergeRequest.iid}: \`${mergeRequest.title}\``,
      ...
      fields: [
        ...
        {
          name: 'Latest Commit',
          value: `[\`${lastCommit.id.slice(0, 7)}\`](${
            lastCommit.url
          }) ${lastCommitMessage} [${lastCommitAuthor}]`,
        },
      ],
    };
  }
...
module.exports = MergeRequestUpdate;
