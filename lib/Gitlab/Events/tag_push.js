...
class TagPush extends EventResponse {
    ...
    embed(data) {
        ...
        return {
            color: 0xf0c330,
            title: `${this.isCreated(data) ? 'Created' : 'Deleted'} tag \`${tag}\` ${sha ? `from commit \`${sha}\`` : ''}${
                isCreated ? ` with ${commitCount} ${commitCount !== 1 ? 'commits' : 'commit'}` : ''
            }`,
    ...
    text(data) {
        ...
        return [
            `⚡ **${actor}** ${isCreated ? 'created' : 'deleted'} tag \`${tag}\` ${sha ? `from commit \`${sha}\`` : ''}${
                isCreated ? `with ${commitCount} ${commitCount !== 1 ? 'commits' : 'commit'}` : ''
            }`,
            `  ${message.split('\n')[0]}`,
            ` `,
