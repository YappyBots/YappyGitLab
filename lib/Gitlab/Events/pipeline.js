...
class Pipeline extends EventResponse {
  ...
  embed(data) {
    ...
    const branch = data.object_attributes.ref;
    const id = data.object_attributes.id;
    const result =
      data.object_attributes.status === 'success' ? 'passed' : 'failed';
  ...
  text(data) {
    ...
    const msg =
      `Pipeline \`#${id}\` of \`${branch}\` branch created by *${actor}* **${result}** in ${duration} seconds. ${icon}\n` +
