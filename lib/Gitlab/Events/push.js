...
class Push extends EventResponse {
  ...
  embed(data) {
    ...
    return {
      color: 0x7289da,
      title: `Pushed ${commitCount} ${
        commitCount !== 1 ? 'commits' : 'commit'
      } to \`${branch}\``,
      url: `${data.project.web_url}/compare/${data.before.slice(
        0,
        7
  ...
  text(data) {
    ...
    let msg = `⚡ **${actor}** pushed ${commitCount} ${
      commitCount !== 1 ? 'commits' : 'commit'
    } to \`${branch}\``;
    let commitArr = commits
      ...
      .slice(0, 5);
    msg += `\n${commitArr.join('\n')}`;
    msg += `\n<${data.project.web_url}/compare/${data.before.slice(
      0,
      7
    )}...${data.after.slice(0, 7)}>`;
