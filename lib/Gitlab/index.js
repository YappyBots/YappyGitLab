...
class Gitlab {
    ...
    getGroupProjects(org) {
        return this._request(Constants.Endpoints.Group(encodeURIComponent(org)).projects);
    }

    _getRepoID(ownerOrId, name) {
        const data = name && parse(`${ownerOrId}/${name}`);
        return encodeURIComponent(data ? data.repo : ownerOrId);
    }

    _request(url, searchParams) {
        return got(url.toString(), { searchParams, responseType: 'json' });
    }
}
