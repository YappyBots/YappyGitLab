const regex =
    /^(?:(?:https?:\/\/|git@)((?:(?:[a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*(?:[A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9]))(?:\/|:))?([\w\.@\:\/\-~]+?)(\.git)?$/i;
...
module.exports = (str) => {
    ...
    parsed.owner = repo[0];
    parsed.group = repo.length > 2 ? repo.slice(1, repo.length - 1) : null;
    parsed.name = repo[repo.length - 1];
    parsed.isGitlab = !parsed.host || parsed.host === 'gitlab.com';
