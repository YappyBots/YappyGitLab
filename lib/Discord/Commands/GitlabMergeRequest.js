        ...
        return Gitlab.getProjectMergeRequest(repository, null, mrNumber)
            .then((res) => {
                ...
                const embed = new this.embed()
                    ...
                    .setColor('#84F139')
                    .addFields([
                        { name: 'Source', value: `${mr.author.username}:${mr.source_branch}`, inline: true },
                        { name: 'Target', value: `${repository.split('/')[0]}:${mr.target_branch}`, inline: true },
                        { name: 'State', value: mr.state === 'opened' ? 'Open' : `${mr.state[0].toUpperCase()}${mr.state.slice(1)}`, inline: true },
                        { name: 'Status', value: mr.work_in_progress ? 'WIP' : 'Finished', inline: true },
                        { name: 'Labels', value: mr.labels.length ? mr.labels.map((e) => `\`${e}\``).join(', ') : 'None', inline: true },
                        { name: 'Milestone', value: mr.milestone ? `${mr.milestone.title}` : 'None', inline: true },
                        { name: 'Comments', value: mr.user_notes_count, inline: true }
                    ])
