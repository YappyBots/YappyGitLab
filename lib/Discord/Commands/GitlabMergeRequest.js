...
class GitlabIssue extends Command {
  constructor(bot) {
    ...
    this.props.help = {
      name: 'mr',
      description:
        'Search merge requests or get info about specific merge request',
      usage: 'mr <list|#> [page]',
  ...
  _mr(msg, args) {
    ...
    if (!repository)
      return this.commandError(
        msg,
        Gitlab.Constants.Errors.NO_REPO_CONFIGURED(this)
      );
    ...
    return Gitlab.getProjectMergeRequest(repository, null, mrNumber)
      .then(res => {
        ...
        const embed = new this.embed()
          ...
          .addField(
            'Target',
            `${repository.split('/')[0]}:${mr.target_branch}`,
            true
          )
          .addField(
            'State',
            mr.state === 'opened'
              ? 'Open'
              : `${mr.state[0].toUpperCase()}${mr.state.slice(1)}`,
            true
          )
          ...
          .addField(
            'Labels',
            mr.labels.length
              ? mr.labels.map(e => `\`${e}\``).join(', ')
              : 'None',
            true
          )
          .addField(
            'Milestone',
            mr.milestone ? `${mr.milestone.title}` : 'None',
            true
          )
        ...
        if (imageUrl)
          embed.setImage(
            imageUrl.startsWith('/')
              ? `https://gitlab.com/${repository}/${imageUrl}`
              : imageUrl
          );
