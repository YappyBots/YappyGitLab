...
class GitlabInitOrgCommand extends Command {
  ...
  async run(msg, args) {
    ...
    .catch(err => {
      ...
      if (errorMessage && errorMessage !== 'Not Found') return this.commandError(msg, `Unable to get group info for \`${orgName}\`\n${err}`);
      return this.commandError(msg, `Unable to initialize! The group \`${orgName}\` doesn't exist!`);
    });
  }
  ...
  _successMessage(org, repos) {
    return {
      ...
      description: [
        'The repositories must all have a webhook pointing to <https://www.yappybots.ga/gitlab>',
        !conf.embed && 'To use embeds to have a nicer Gitlab log, say `G! conf set embed true` in this channel.',
