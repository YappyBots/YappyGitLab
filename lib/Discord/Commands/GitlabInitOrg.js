...
class GitlabInitOrgCommand extends Command {
    ...
    async run(msg, args) {
        ...
        return Gitlab.getGroupProjects(orgName)
            ...
            .then((r) => workingMsg.edit({ embeds: [this._successMessage(orgName, r, conf.get('useEmbed'))] }))
            .catch((err) => {
    ...
    _successMessage(org, repos, hasEmbed) {
        const embed = new this.embed()
            ...
            .setTitle(`\`${org}\`: Successfully initialized all public repository events`)
            .setDescription(
                [
                    'The repositories must all have a webhook pointing to <https://yappy.dsev.dev/gitlab>',
