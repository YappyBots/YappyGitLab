...
class ConfCommand extends Command {
    ...
    async run(msg, args) {
        ...
        const channelConf = await Channel.find(msg.channel.id, ['guild', 'repos']);
    ...
    _view(msg, args, channelConf, serverConf) {
        ...
        if (args.includes('--global') || args.includes('-g')) {
            let embed = new this.embed()
                ...
                .addField('Prefix', serverConf.has('prefix') ? this.format(serverConf.get('prefix')) : '`GL! \u200B`', true);
        ...
        let embed = new this.embed()
            ...
            .addField('Repos', this.formatArrayOutput(channelConf.getRepos()))
            .addField('Repo (repo)', this.format(channelConf.get('repo')), true)
            .addField('Use Embed (useEmbed)', this.format(channelConf.get('useEmbed'), true), true)
            .addField(`Events [${channelConf.get('eventsType')}]`, this.format(channelConf.get('eventsList')), true)
            .addField(`Users [${channelConf.get('usersType')}]`, this.format(channelConf.get('usersList')), true)
            .addField(`Branches [${channelConf.get('branchesType')}]`, this.format(channelConf.get('branchesList')), true);
    ...
    _get(msg, args, channelConf, serverConf) {
        ...
        const embed = new this.embed()
            ...
            .addField(key, this.format(channelConf[key]));
        ...
        return conf
            ...
            .then(() => {
                const embed = new this.embed()
                    ...
                    .addField(key, this.format(value, conf.casts && conf.casts[key] === 'boolean'));
                return msg.channel.send({ embed });
            })
            .catch(err => {
                const embed = new this.embed()
                    .setColor('#CE0814')
                    .setAuthor(embedData.author, guild.iconURL)
                    .setDescription([`An error occured when trying to update ${embedData.confName}'s configuration`, '```js', err, '```'].join('\n'))
                    .addField(key, value);
                return msg.channel.send({ embed });
            });
    }
    ...
    _events(msg, [, , a, ...args], conf) {
        ...
        if (action === Actions.INVALID) {
            if (!a) {
                ...
                return this.commandError(
                    msg,
                    `Correct Usage: \`${this.bot.prefix}conf filter events [list|ignore|disable] [event]\``,
                    'Incorrect usage'
                );
            }
        }
