...
class ConfCommand extends Command {
  ...
  async run(msg, args) {
    ...
    else if (action === 'get') return this._get(msg, args, channelConf, serverConf);
    else if (action === 'set') return this._set(msg, args, channelConf, serverConf);
  ...
  _set(msg, a, channelConf, serverConf) {
    ...
    let value = global ? args.filter(e => !e.includes('-g')).slice(2).join(' ') : args.slice(2).join(' ');
  ...
  _events(msg, [, a, ...args], conf) {
    ...
    if (action === Actions.INVALID) {
      if (!a || a.toLowerCase() === 'view') {
        ...
        const embed = new this.embed()
        .setTitle('List of available events');
    ...
    if (!EventHandler.eventsList.has(key.replace('/', '-'))) return this.commandError(msg, 'Invalid event name provided');
