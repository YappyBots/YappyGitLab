...
class ConfCommand extends Command {
    ...
    _set(msg, a, channelConf, serverConf) {
        ...
        const embed = new this.embed().setAuthor(embedData.author, guild.iconURL());
    ...
    async _filter(msg, [, obj, cmd, ...args], conf) {
        ...
        if (/^(whitelist|blacklist)$/i.test(cmd)) {
            ...
            return msg.channel.send(
                new this.embed().setColor('#84F139').setDescription(`Successfully updated #${msg.channel.name}'s filtering mode for ${obj} to ${cmd}`)
            );
        }
    ...
    _events(msg, [, , a, ...args], conf) {
        ...
        if (action === Actions.INVALID) {
            if (!a) {
                ...
                events.forEach((e) => e && embed.addField(e, '\u200B', true));
