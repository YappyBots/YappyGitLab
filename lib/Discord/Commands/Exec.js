...
class ExecCommand extends Command {
    ...
    run(msg, args) {
        ...
        let runningMessage = ['`RUNNING`', '```xl', this._clean(command), '```'].join('\n');
        ...
        return msg.channel
            ...
            .then((stdout) => {
                ...
                let message = ['`STDOUT`', '```sh', this._clean(stdout) || ' ', '```'].join('\n');
            ...
            .catch((data) => {
                ...
                let message = ['`STDOUT`', '```sh', this._clean(stdout) || '', '```', '`STDERR`', '```sh', this._clean(stderr) || '', '```']
                    .join('\n')
                    .substring(0, 2000);

                if (messageToEdit) messageToEdit.edit(message);
