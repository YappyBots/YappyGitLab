...
class GitlabRemoveCommand extends Command {
    ...
    async run(msg, args) {
        ...
        const workingMsg = await msg.channel.send({
            embeds: [new this.embed().setColor(0xfb9738).setTitle(`\`${repoFound}\`: ⚙ Working...`)],
        });

        return ChannelRepo.where('channel_id', conf.id)
            .where('name', repoFound)
            .destroy()
            .then(() => workingMsg.edit({ embeds: [this._successMessage(repoFound)] }))
            .catch((err) => {
                Log.error(err);
    ...
    _successMessage(repo) {
        return new this.embed()
            ...
            .setTitle(`\`${repo}\`: Successfully removed repository events`);
    }
}
