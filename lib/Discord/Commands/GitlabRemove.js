...
class GitlabRemoveCommand extends Command {
  ...
  async run(msg, args) {
    ...
    const conf =
      ChannelConfig.get(channelid) || (await ChannelConfig.add(msg.channel));
    ...
    if (!conf.repos || !conf.repos[0]) {
      return this.commandError(
        msg,
        "This channel doesn't have any GitLab events!"
      );
    }
    ...
    } else if (conf.repos.length === 1) {
      repoFound = conf.repos[0];
    }

    if (args[0] && !repoFound) {
      return this.commandError(
        msg,
        `This channel doesn't have GitLab events for **${repo.repo ||
          args[0]}**!`
      );
    } else if (conf.repos.length && conf.repos.length > 1 && !repoFound) {
      return this.commandError(
        msg,
        `Specify what GitLab repo event to remove! Current repos: ${conf.repos
          .map(e => `**${e}**`)
          .join(', ')}`
      );
    }
