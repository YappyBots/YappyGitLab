  ...
  get help() {
    return {
      ...
      domain: this.textToEmbed('Yappy Bots domain has expired', [
        ...
        'The new domain is https://www.yappybots.ga, which will hopefully be temporary until we are able to get our old one back.',
  ...
  run(msg, args, next) {
    ...
    .then((res) => {
      ...
      if (Date.now() - this.cooldowns.get(msg.channel.id) < 15000) {
        return this.moduleError(msg, `There are ${this.calculateCooldownLeft(msg)} left until the response will be sent again.`);
      }

      if (action === 'help.general') msg.channel.send({ embed: helpMessages.general });
      if (action === 'help.webhook') msg.channel.send({ embed: helpMessages.webhook });
      if (action === 'help.domain') msg.channel.send({ embed: helpMessages.domain });
