    ...
    fs.readdir(cwd, (err, files) => {
      ...
      files.forEach(f => {
        try {
          ...
          if (disabled.includes(Command.help.name))
            return Log.info(`Skipping command: ${Command.help.name}`);
  ...
  async execute(msg) {
    ...
    const serverConf =
      msg.guild && msg.guild.available
        ? await ServerConfig.forServer(msg.guild)
        : null;
    ...
    const content =
      ...
      (msg.content.startsWith(this.user.toString()) &&
        msg.content.replace(`${this.user.toString()} `, '')) ||
      (msg.content.startsWith(this.prefix) &&
        msg.content.replace(this.prefix, '')) ||
      msg.content;
