...
app.post('/', async (req, res) => {
    ...
    const handleError = (resp, channel) => {
        ...
        if (!res || !err) return;
        if (channel?.guild?.owner && (errors.includes(err.message) || (err.error && errors.includes(err.error.message)))) {
    ...
    const branch = data.ref ? GetBranchName(data.ref) : data.object_attributes && data.object_attributes.ref;

    channels.forEach(async (conf) => {
        const wantsEmbed = conf.get('useEmbed');
        const channel = await bot.channels.fetch(conf.id).catch((err) => {
            if (err.message === 'Unknown Channel') {
                Log.warn(`Unable to fetch channel ${conf.id} -- ${err.name} ${err.message} (${err.status})`);
            } else {
                Log.error(err);
            }
        });
        ...
        try {
            if (wantsEmbed) await channel.send({ embeds: [eventResponse.embed] });
...
app.use(
    addons.express.middleware(
        ...
        },
        {
            ...
            host: process.env.BDPW_KEY ? 'https://yappy.dsev.dev/gitlab' : `http://localhost:${port}`,
        }
    )
);
