...
app.post('/', (req, res) => {
  ...
  if (!event || !data || !data.project) {
    return res.status(403).send('Invalid data. Plz use Gitlab webhooks.');
  }
  ...
  Log.verbose(
    `GitLab | ${repo} - ${eventName}${actionText} (${channels.size} channels)`
  );
  res.send(
    `${data.project.path_with_namespace} : Received ${eventName}${actionText}, emitting to ${channels.size} channels...`
  );
  ...
  if (!eventResponse) {
    return res
      .status(500)
      .send('An error occurred when generating the Discord message');
  }
  if (!eventResponse.embed && !eventResponse.text) {
    return Log.warn(`GitLab | ${repo} - ${eventName}${actionText} ignored`);
  }
  ...
  const handleError = (resp, channel) => {
    ...
    if (
      ...
      channel.guild.owner.send(
        `**ERROR:** Yappy GitLab doesn't have permissions to read/send messages in ${channel}`
      );
    } else {
      channel.guild.owner.send(
        [
  ...
  channels.forEach(conf => {
    ...
    const branch = data.ref
      ? GetBranchName(data.ref)
      : data.object_attributes.ref;

    if (!channel) return;
    if (
      disabledEvents.includes(eventName) ||
      disabledEvents.includes(`${eventName}${actionText}`)
    ) {
      return;
    }
    if (
      ignoredUsers &&
      (ignoredUsers.includes(actor.name) || ignoredUsers.includes(actor.id))
    ) {
      return;
    }
